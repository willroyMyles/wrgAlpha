...
const concurrencyLimit_js_1 = require("./concurrencyLimit.js");
const errors_js_1 = require("./errors.js");
const httpEndpoint_js_1 = require("./httpEndpoint.js");
const io_js_1 = require("./io.js");
const ioWithIntegrations_js_1 = require("./ioWithIntegrations.js");
...
const dynamic_js_1 = require("./triggers/dynamic.js");
const eventTrigger_js_1 = require("./triggers/eventTrigger.js");
const scheduled_js_1 = require("./triggers/scheduled.js");
const formatSchemaErrors_js_1 = require("./utils/formatSchemaErrors.js");
const version_js_1 = require("./version.js");
const parseRequestPayload = (rawPayload) => {
    const result = core_1.RequestWithRawBodySchema.safeParse(rawPayload);
    if (!result.success) {
        throw new errors_js_1.ParsedPayloadSchemaError((0, formatSchemaErrors_js_1.formatSchemaErrors)(result.error.issues));
    }
...
const registerWebhookEvent = (key) => ({
    name: `${core_1.REGISTER_WEBHOOK}.${key}`,
    title: "Register Webhook",
    source: "internal",
    icon: "webhook",
    parsePayload: core_1.RegisterWebhookPayloadSchema.parse,
});
const registerSourceEvent = {
    name: core_1.REGISTER_SOURCE_EVENT_V2,
    title: "Register Source",
    source: "internal",
    icon: "register-source",
    parsePayload: core_1.RegisterSourceEventSchemaV2.parse,
};
...
class TriggerClient {
    ...
    constructor(options) {
        ...
        this.#internalLogger = new logger_1.Logger("trigger.dev", this.#options.verbose ? "debug" : "log", [
            "output",
            "noopTasksSet",
        ]);
        this.#client = new apiClient_js_1.ApiClient({
            logLevel: this.#options.verbose ? "debug" : "log",
            ...this.#options,
        });
        this.#envStore = new keyValueStore_js_1.KeyValueStore(this.#client);
    }
    ...
    async handleRequest(request, timeOrigin = performance.now()) {
        ...
        switch (action) {
            ...
            case "INITIALIZE_TRIGGER": {
                ...
                const body = core_1.InitializeTriggerBodySchema.safeParse(json);
                if (!body.success) {
                    return {
                        status: 400,
                        body: {
            ...
            case "EXECUTE_JOB": {
                const json = await request.json();
                const execution = core_1.RunJobBodySchema.safeParse(json);
            ...
            case "PREPROCESS_RUN": {
                const json = await request.json();
                const body = core_1.PreprocessRunBodySchema.safeParse(json);
                if (!body.success) {
                    return {
                        status: 400,
                        body: {
            ...
            case "DELIVER_HTTP_SOURCE_REQUEST": {
                const headers = core_1.HttpSourceRequestHeadersSchema.safeParse(Object.fromEntries(request.headers.entries()));
                if (!headers.success) {
                    return {
                        status: 400,
                        body: {
            ...
            case "DELIVER_HTTP_ENDPOINT_REQUEST_FOR_RESPONSE": {
                const headers = core_1.HttpEndpointRequestHeadersSchema.safeParse(Object.fromEntries(request.headers.entries()));
                if (!headers.success) {
                    return {
                        status: 400,
                        body: {
            ...
            case "DELIVER_WEBHOOK_REQUEST": {
                const headers = core_1.WebhookSourceRequestHeadersSchema.safeParse(Object.fromEntries(request.headers.entries()));
                if (!headers.success) {
                    return {
                        status: 400,
                        body: {
    ...
    defineJob(options) {
        ...
        const job = new job_js_1.Job(options);
        this.attach(job);
        return job;
    }
    ...
    defineDynamicSchedule(options) {
        return new scheduled_js_1.DynamicSchedule(this, options);
    }
    defineDynamicTrigger(options) {
        return new dynamic_js_1.DynamicTrigger(this, options);
    }
    /**
    ...
    defineHttpEndpoint(options, suppressWarnings = false) {
        ...
        const endpoint = (0, httpEndpoint_js_1.httpEndpoint)(options);
        this.#registeredHttpEndpoints[endpoint.id] = endpoint;
        return endpoint;
    }
    defineConcurrencyLimit(options) {
        return new concurrencyLimit_js_1.ConcurrencyLimit(options);
    }
    attach(job) {
    ...
    attachDynamicTrigger(trigger) {
        ...
        this.defineJob({
            ...
            version: trigger.source.version,
            trigger: new eventTrigger_js_1.EventTrigger({
    ...
    authorized(apiKey) {
        ...
        const localApiKey = this.#options.apiKey ?? node_process_1.env.TRIGGER_API_KEY;
        if (!localApiKey) {
    ...
    apiKey() {
        return this.#options.apiKey ?? node_process_1.env.TRIGGER_API_KEY;
    }
    ...
    async #executeJob(body, job, timeOrigin, triggerVersion) {
        ...
        const io = new io_js_1.IO({
            ...
            jobLogger: this.#options.ioLogLocalEnabled
                ? new logger_1.Logger(job.id, job.logLevel ?? this.#options.logLevel ?? "info")
                : undefined,
        ...
        const ioWithConnections = (0, ioWithIntegrations_js_1.createIOWithIntegrations)(io, resolvedConnections.data, job.options.integrations);
        try {
            const parsedPayload = job.trigger.event.parsePayload(body.event.payload ?? {});
            ...
            const output = await runLocalStorage_js_1.runLocalStorage.runWith({ io, ctx: context }, () => {
            ...
            if (error instanceof errors_js_1.ResumeWithParallelTaskError) {
                return {
    ...
    #convertErrorToExecutionResponse(error, body) {
        if (error instanceof errors_js_1.AutoYieldExecutionError) {
            return {
        ...
        if (error instanceof errors_js_1.AutoYieldWithCompletedTaskExecutionError) {
            return {
        ...
        if (error instanceof errors_js_1.YieldExecutionError) {
            return { status: "YIELD_EXECUTION", key: error.key };
        }
        if (error instanceof errors_js_1.ParsedPayloadSchemaError) {
            return { status: "INVALID_PAYLOAD", errors: error.schemaErrors };
        }
        if (error instanceof errors_js_1.ResumeWithTaskError) {
            return { status: "RESUME_WITH_TASK", task: error.task };
        }
        if (error instanceof errors_js_1.RetryWithTaskError) {
            return {
        ...
        if (error instanceof errors_js_1.CanceledWithTaskError) {
            return {
                status: "CANCELED",
                task: error.task,
            };
        ...
        if (error instanceof errors_js_1.ErrorWithTask) {
            const errorWithStack = core_1.ErrorWithStackSchema.safeParse(error.cause.output);
        ...
        if (error instanceof errors_js_1.RetryWithTaskError) {
            const errorWithStack = core_1.ErrorWithStackSchema.safeParse(error.cause);
            if (errorWithStack.success) {
                return {
                    status: "ERROR",
                    error: errorWithStack.data,
        ...
        const errorWithStack = core_1.ErrorWithStackSchema.safeParse(error);
        if (errorWithStack.success) {
    ...
    #standardResponseHeaders(start) {
        return {
            ...
            "Trigger-SDK-Version": version_js_1.VERSION,

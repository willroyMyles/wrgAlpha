...
class ServiceDetailsState extends GetxController {
  ...
  onView() {
    ...
    Future.delayed(const Duration(seconds: 3), () {
      var idx = Get.find<ServiceState>().posts.indexOf(model);

      Get.find<ServiceState>().posts[idx] = model;
    });
  }
  ...
  onWatching() async {
    ...
    //add to watching
    if (res) {
      var state = Get.find<ServiceState>();
  ...
  void updateStatus(Status e) async {
    ...
    if (res) {
      model.status = e;
      GF<ServiceState>().posts.where((e) => e.id == model.id).first.status = e;

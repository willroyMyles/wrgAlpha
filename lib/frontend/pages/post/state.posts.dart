...
class PostState extends ListState<PostModel> {
  ...
  Future<List<PostModel>> getModel() async {
    return await Get.find<GE>().posts_getPosts(limit: limit);
  }

  @override
  Future<List<PostModel>> getMoreModel() async {
    return await Get.find<GE>().posts_getPosts(
        id: list.lastOrNull?.createdAt?.millisecondsSinceEpoch, limit: limit);
  }
}
